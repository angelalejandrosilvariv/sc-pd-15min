...
# Momento en que se trunca a cero el margen que no alcanza a autocubrir el costo.
# Ambos criterios liquidan por ciclo; lo que cambia es CUANDO se descarta lo negativo.
#   0 = trunca bloque a bloque antes de sumar: un cuarto de hora con CMg < CV aporta
#       0 y nunca le resta a otro que si tuvo margen. Es lo que hace el modelo
#       horario (hoja 'Sobrecosto_PD xHyC', columna Y = IF(CMg-CV<0, 0, CMg-CV),
#       truncada por fila hora-central antes del SUMIF del ciclo).
#   1 = deja que los bloques negativos compensen a los positivos DENTRO del ciclo y
#       trunca recien el total: Margen_Suma_Ciclo = MAX(0, suma con signo). Mide la
#       autocobertura sobre la misma unidad en que se cobra la partida/detencion.
# CAMBIA EL MONTO LIQUIDADO. Ver docs/specs/23-margen-neteado-por-ciclo.md para la
...
def compactar_resumen_ciclos(resumen_relacionada, usar_tarifa_rio_instruida,
    ...
    compacto = resumen_relacionada.groupby(
        ['Etiqueta_Relacionada', 'Central_Relacionada', 'Ciclo_ID_Relacionada'],
        as_index=False,
    ).agg(**agregaciones)

    # Con MARGEN_NETEADO_POR_CICLO=1 los bloques llegan con signo desde
    # calcular_margen_bloques(), asi que la suma de arriba ya es el neto del ciclo.
    # Se guarda ese neto para auditoria y se trunca lo que queda bajo cero: sin este
    # MAX, un ciclo con margen neto negativo cobraria mas que su propio costo P-D.
...
def calcular_margen_bloques(reporte, calcular_en_motor=1, netear_por_ciclo=0):
    """Calcula el margen por bloque.
    ...
    netear_por_ciclo = 0: el bloque con margen unitario negativo aporta 0, de modo
        que nunca le resta a otro bloque del mismo ciclo (criterio del modelo
        horario). El resultado es cero o positivo por bloque.
    netear_por_ciclo = 1: conserva el signo del bloque para que se compense dentro
        del ciclo. El truncamiento lo hace despues compactar_resumen_ciclos() sobre
        el total ya neteado, no aqui.
    """
    ...
    if netear_por_ciclo == 1:
        return np.asarray(margen_unitario * generacion, dtype=float)
...
def crear_guia_lectura():
    ...
    filas = [
        ...
        ('Margen_Suma_Ciclo', 'Margen del ciclo ya truncado en cero, que descuenta del costo P-D. '
         'Con MARGEN_NETEADO_POR_CICLO=0 es la suma de los bloques positivos (criterio del modelo horario); '
         'con 1 es MAX(0, suma con signo de todos los bloques del ciclo).'),
        ('Margen_Neto_Ciclo', 'Solo con MARGEN_NETEADO_POR_CICLO=1: suma CON SIGNO del margen de los bloques, '
         'antes de truncar. Si es negativo, el ciclo no se autocubrio y vendio energia a perdida; '
         'Margen_Suma_Ciclo queda en 0 y el costo P-D se paga completo.'),
        ('Formula completa',
...
def main(rutas: dict, panel: dict | None = None, devolver_diagnostico: bool = False):
    ...
    print("\n  TRUNCAMIENTO DEL MARGEN NEGATIVO")
    negativos = int((reporte_sin_ceros['Margen'] < 0).sum())
    monto_negativo = float(reporte_sin_ceros.loc[
        reporte_sin_ceros['Margen'] < 0, 'Margen'].sum())
    if MARGEN_NETEADO_POR_CICLO == 1:
        print("  ACTIVO: neteado POR CICLO -> los bloques negativos compensan a los")
        print("          positivos del mismo ciclo y el total se trunca en cero.")
        print(f"  Bloques con margen negativo que si compensan: {negativos:,} "
              f"({monto_negativo:,.0f} CLP)")
        print("  ATENCION: este criterio NO es el del modelo horario. Ver")
        print("            docs/specs/23-margen-neteado-por-ciclo.md")
    else:
        print("  ACTIVO: truncado POR BLOQUE -> cada bloque con CMg < CV aporta 0 y")
        print("          nunca le resta a otro bloque (criterio del modelo horario).")
        print("  Cambia MARGEN_NETEADO_POR_CICLO=1 para netear dentro del ciclo.")
