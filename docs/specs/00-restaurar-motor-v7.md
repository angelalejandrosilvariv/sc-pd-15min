# Spec 00 — Restaurar el código real del motor v7
...
El repositorio quedó con un archivo `src/sc_pd_motor_v7.py` que es solo un
esqueleto de 14 líneas (imports + un comentario que dice "Original v7 source
is preserved below", sin el código real a continuación). El motor real —
la v7, con las correcciones de los bugs 1 a 7 documentadas en su propio
encabezado — existe fuera del repo. El dueño del proyecto lo entregó y esta
spec transcribe ese archivo tal cual, sin modificar ninguna línea.

## Instrucción

Reemplaza **todo** el contenido de `src/sc_pd_motor_v7.py` por el bloque de
código que aparece más abajo, de forma literal:

- No reformatees, no "mejores" nada, no cambies nombres de variables ni
  rutas del panel de control.
- No corrijas los bugs que se mencionan en `01-fase1-integridad-datos.md`
  todavía — eso es una spec separada, a implementar después de este commit.
- Conserva el encabezado/docstring completo (incluye la bitácora de bugs
  1-7, que es documentación funcional del propio archivo, no un comentario
  decorativo).
- El archivo usa rutas de ejemplo tipo Windows (`C:\...`) en el panel de
  control de la sección 0 — son intencionalmente placeholders que cada
  usuario ajusta antes de correr el script. No los cambies a rutas Linux ni
  las conviertas a `Path(...)`.

## Verificación antes de commitear

1. `python -c "import ast; ast.parse(open('src/sc_pd_motor_v7.py', encoding='utf-8').read())"`
   no debe lanzar `SyntaxError`.
2. `wc -l src/sc_pd_motor_v7.py` debe dar aproximadamente 1660 líneas (no 14).
3. El archivo debe seguir empezando con el docstring `# -*- coding: utf-8 -*-`
   y terminar en la línea `print("Listo. Proceso finalizado.")`.

## Código a pegar en `src/sc_pd_motor_v7.py`
...
if RUTA_REPORTE_MES_PASADO and os.path.exists(RUTA_REPORTE_MES_PASADO):
    ...
    reporte = pd.concat([reporte_pasado, reporte_actual], ignore_index=True)

    llave_dedup = ['FECHA_HORA', 'UNIDAD GENERADORA', 'Central', 'CONFIGURACION']
    n_dup = reporte.duplicated(subset=llave_dedup).sum()
    if n_dup:
        gen_dup = reporte[reporte.duplicated(subset=llave_dedup)]['GENERACION'].sum()
        print(f"  [!] {n_dup:,} filas duplicadas tras el empalme ({gen_dup:,.2f} MWh). Se eliminan.")
        reporte = reporte.drop_duplicates(subset=llave_dedup, keep='last').reset_index(drop=True)
    print("  Empalme temporal realizado.")
else:
...
def calcular_ciclos_por_nivel(df, nivel_agrupacion, nombre_columna_id):
    """
    Numera los ciclos de generacion de cada grupo.

    [BUG 1] shift() sobre una Serie booleana devuelve dtype object. En ese caso
    ~Serie aplica el operador bitwise sobre bools de Python (~True = -2,
    ~False = -1), y ambos resultados son truthy: todas las filas quedaban
    marcadas como inicio de ciclo. fill_value=False evita pasar por object.
    """
    columnas_clave = list(nivel_agrupacion) + ['FECHA_HORA']
    df_nivel = df.groupby(columnas_clave, as_index=False)['GENERACION'].sum()
    df_nivel = df_nivel.sort_values(by=columnas_clave).reset_index(drop=True)

    df_nivel['Generando_Bruto'] = (df_nivel['GENERACION'].fillna(0) > 0).astype(bool)

    if TOLERANCIA_CORTES_BLOQUES > 0:
        df_nivel['Generando'] = (
            df_nivel.groupby(nivel_agrupacion)['Generando_Bruto']
            .transform(lambda x: x.replace(False, pd.NA)
                                  .ffill(limit=TOLERANCIA_CORTES_BLOQUES)
                                  .fillna(False))
            .astype(bool)
        )
    else:
        df_nivel['Generando'] = df_nivel['Generando_Bruto']

    # <<< CORRECCION CLAVE >>>
    df_nivel['Generando_Previo'] = (
        df_nivel.groupby(nivel_agrupacion)['Generando'].shift(1, fill_value=False).astype(bool)
    )
    df_nivel['Inicio_Ciclo'] = df_nivel['Generando'] & (~df_nivel['Generando_Previo'])

    df_nivel[nombre_columna_id] = df_nivel.groupby(nivel_agrupacion)['Inicio_Ciclo'].cumsum()
    df_nivel[nombre_columna_id] = np.where(df_nivel['Generando_Bruto'], df_nivel[nombre_columna_id], 0)

    return df_nivel[columnas_clave + [nombre_columna_id]]
...
rio_subset = rio[rio_cols].copy()
rio_subset['_prio'] = rio_subset['MOTIVO'].notna().astype(int)
rio_subset = (rio_subset.sort_values(['FECHA_HORA_RIO', 'Central_Relacionada_RIO', '_prio'])
                        .drop_duplicates(subset=['FECHA_HORA_RIO', 'Central_Relacionada_RIO'], keep='last')
                        .drop(columns=['_prio'])
...
resumen_relacionada = resumen_relacionada.rename(columns={'NOMBRE CONFIGURACIÓN': 'Configuracion RIO'}) \
                                         .drop(columns=['FECHA_HORA_RIO', 'Central_Relacionada_RIO'],
                                               errors='ignore')
...
df_compacto['Costo_Partida_Efectivo'] = (df_compacto['Costo_Partida_Base']
                                         * df_compacto['Filtro_Conf_Partida']
                                         * df_compacto['Filtro_Disp_Partida']
                                         * df_compacto['Filtro_Op_Partida'])
df_compacto['Costo_Detencion_Efectivo'] = (df_compacto['Costo_Detencion_Base']
                                           * df_compacto['Filtro_Conf_Detencion']
                                           * df_compacto['Filtro_Disp_Detencion']
                                           * df_compacto['Filtro_Op_Detencion'])
...
df_compacto['Obs_Partida'] = np.select(
    [df_compacto['Config_RIO_Sin_Tarifa_Partida'],
     df_compacto['Filtro_Conf_Partida'] == 0,
     df_compacto['Filtro_Disp_Partida'] == 0,
     df_compacto['Filtro_Op_Partida'] == 0,
     df_compacto['Costo_Partida_Base'] == 0],
    ['Revisar manualmente: config RIO sin tarifa', 'Rechazo: Configuracion RIO distinta',
     'Rechazo: Maquina en Pruebas (EP)', 'Rechazo: Sin Motivo ni SSCC en RIO',
     'Sin tarifa de partida'], default='Aprobado')

df_compacto['Obs_Detencion'] = np.select(
    [df_compacto['Config_RIO_Sin_Tarifa_Detencion'],
     df_compacto['Filtro_Conf_Detencion'] == 0,
     df_compacto['Filtro_Disp_Detencion'] == 0,
     df_compacto['Filtro_Op_Detencion'] == 0,
     df_compacto['Costo_Detencion_Base'] == 0],
    ['Revisar manualmente: config RIO sin tarifa', 'Rechazo: Configuracion RIO distinta',
     'Rechazo: Maquina en Pruebas (EP)', 'Rechazo: Sin Motivo ni SSCC en RIO',
     'Sin tarifa de detencion'], default='Aprobado')
...
```
Restaurar el código real del motor v7 en src/sc_pd_motor_v7.py

El archivo tenía solo el esqueleto de imports; se reemplaza por el
código v7 completo entregado por el dueño del proyecto (correcciones
de bugs 1-7 documentadas en su propio encabezado).
```
