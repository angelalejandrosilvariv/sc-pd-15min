...
### 2026-09-16 — Codex (OpenAI) — Paquete de auditoría CEN (spec 31)

- **Tipo:** implementación, trazabilidad, documentación y pruebas.
- **Origen:** `docs/specs/31-entrega-auditoria-cen.md`.
- **Cambios:** generador de entrega con seis CSV y libro auditable, hashes SHA-256,
  commit e interruptores; función no invasiva para candidatas de tarifa; runner y
  botón de interfaz. El detalle expone `Dolar` junto con los insumos CMg/CV existentes.
- **Validación:** `pytest -q`, compilación de scripts y `git diff --check`.
- **Pendientes:** Claude debe validar agosto real y confirmar ambos checks en cero.
